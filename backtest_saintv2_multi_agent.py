...
class LiveConfig:
    ...
    tp_shrink: float = 1.0  # pas de shrink (atr_tp_mult contient déjà le facteur final)
    ...
    spread_bps: float = 0.0002    # fraction, pas des bps : 0.0002 = 2 bps
    ...
    # 0.40 = aligné avec CONF_THRESHOLD (training), loup_live et le MQL5.
    ...
    # Agents à backtester — aligné avec LiveConfig.active_agents de kairos_live.
    # None = tous ceux de MULTI_AGENT_PATHS (exige que les 3 .pth existent).
...
class StressConfig:
    ...
    # Calibré pour égaler le training : abs(N(0,σ))×1.5 → moyenne σ×1.197 = 1.5 bps.
...
def apply_price_stress_to_bar(
    ...
    # 1b) Expansion asymétrique du range — wicks intra-minute que les bars M1
    # agrégées ratent. Ce bloc manquait ici : ce backtest était donc plus
    # permissif que training et que no_be_trail sur le déclenchement des SL.
